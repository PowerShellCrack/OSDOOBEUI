...
(WPFVar "inputCmbDomainWorkgroupName").Add_SelectionChanged({
    ...
    If((WPFVar "inputCmbDomainWorkgroupName").SelectedItem -ne 'Workgroup')
    {
        ...
        Else{
            ...
            $SelectedDomainName = ($MenuLocaleDomainList | Where FQDN -eq (WPFVar "inputCmbDomainWorkgroupName").SelectedItem).Name + $AddSlash
        }
        #replace domain with new domain for creds
        (WPFVar "inputTxtDomainAdminLocalAccount").text = ($CurrentUserValue -replace $OldDomain,$SelectedDomainName)
    }
    Else
    {
        #move Domain join down & hide Domain OU
...
#Event handler trigger when cursor is moved away from inputer Domain Name field instead of textchanged event
...
$UI.Add_KeyDown({
    ...
    If ([System.Windows.Input.Keyboard]::IsKeyDown("RightCtrl") -OR [System.Windows.Input.Keyboard]::IsKeyDown("LeftCtrl")) {
        Write-Host "Ctrl key presss"
        Switch ($Key) {
            "LEFT" {
                Write-Host "left arrow pressed"
                Switch-TabItem -TabControlObject (WPFVar "subtabControl") -name 'Hardware'
            }
            "RIGHT" {
                Write-Host "right arrow pressed"
...
If(!$TestMode)
{
    If($MenuShowSplashScreen){Show-UISplashScreenProgress -Label ("Loading User Interface" -f $MenuVersion) -Progress 100}
    Show-UIMenu
    If($MenuShowSplashScreen){Close-UISplashScreen}
}
Else
{
    Show-UICommandHelp -ExampleText $NameStandardRuleExampleText
}